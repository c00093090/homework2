...
```{r ans22}
library(lubridate)
commit8Date01 <- ymd_hms(c(commit8Date))
class(commit8Date01)
commit8TWDate <- with_tz(commit8Date01, tz="Asia/Taipei")
commit8TWDate
```
...
```{r ans23}
length(commitCheck)
names(commitCheck)[c(1:30)] <- c("commit1","commit2","commit3", "commit4","commit5",
                                 "commit6","commit7", "commit8","commit9","commit10",
                                 "commit11","commit12","commit13", "commit14","commit15",
                                 "commit16","commit17", "commit18","commit19","commit20",
                                 "commit21","commit22","commit23", "commit24","commit25",
                                 "commit26","commit27", "commit28","commit29","commit30")
commitCheck
```
...
```{r ans25}
library(lubridate)
allCommitDates01 <- ymd_hms(c(allCommitDates))
class(allCommitDates01)
allCommitDatesTW <- with_tz(allCommitDates01, tz="Asia/Taipei")
allCommitDatesTW
```
...
library(stringr)
library(lubridate)
hour01 <- hour(allCommitDatesTW)
min01 <- minute(allCommitDatesTW)
max01 <- str_c(hour01,min01)
logicWithinTimeFrame <- (max01<=630 & max01>=430)
...
```{r ans32}
library(stringr)
which(variableNames=="學系")
t1<- transcriptData[3]
logicFromEcon <-(t1 == "經濟學系" )
logicFromEcon
```
...
```{r ans34}
library(stringr)
transcriptPartialDataEcon <- list("學號"=transcriptPartialData[1][logicFromEcon],
                                  "學分數"=transcriptPartialData[2][logicFromEcon],
                                  "必選修類別（必∕選∕通）"=transcriptPartialData[3][logicFromEcon])
class(transcriptPartialDataEcon)
